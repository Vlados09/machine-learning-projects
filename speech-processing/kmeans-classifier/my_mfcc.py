...
class MFCC:
	...
	def __init__(self, file, num_ceps=12, use_hamming=True, num_filters=26, use_preemph=True, normalize=False,
	             deltas=False):
		...
		self.normalize = normalize           # Determines if mean normalization should be performed
		self.deltas = deltas
		
	def get_mffc(self, verbose=False):
		...
		if self.normalize:
			mfcc_out = (mfcc - np.mean(mfcc, axis=0))/np.std(mfcc, axis=0)
			if verbose:
				print("=== After normalisation")
				print(f"mfcc mean = {np.round(np.mean(mfcc_out, axis=0), 2)}")
				print(f"mfcc std = {np.round(np.std(mfcc_out, axis=0), 2)}")
		else:
			mfcc_out = mfcc
		
		if verbose:
			self.plot_results(powspec, fbank, mfcc, mfcc_out)
		
		"""if self.deltas:
			# Compute MFCC deltas and delta deltas:
			mfcc_delta = delta(mfcc_out)
			mfcc_delta_2 = delta(mfcc_out, order=2)
			mfcc_out = np.hstack([mfcc_out, mfcc_delta, mfcc_delta_2])"""
